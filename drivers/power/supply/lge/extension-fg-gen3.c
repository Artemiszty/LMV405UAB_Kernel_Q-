...
#define LOOP_TIME_S		5

#define CHANGE_POINT		100
#define SCALING_FACTOR		2
#define CHANGE_POINT_INC	200
#define SCALING_FACTOR_INC	4
...
#define TEMP_UPDATE_TIME_SEC		3
#define DIVIDE_TEMP			10
#define DIVIDE_MS_TO_S			1000

static int catchup_batt_therm(struct smooth_param *smooth_therm, int temp, int ts_diff)
{
	int catchup_time_s = 0, catchup_temp = 0, change_therm = 0;

	change_therm = abs(temp - smooth_therm->therm_backup);
	...
	if (ts_diff <= catchup_time_s) {
		catchup_temp = ((catchup_time_s - ts_diff)*smooth_therm->therm_backup + (ts_diff * temp))/catchup_time_s;

		pr_info("catchup_temp ts_diff:%d, catchup_time_s:%d, "
			"temp:%d, therm_backup:%d, result_temp:%d\n",
			ts_diff, catchup_time_s,
			temp, smooth_therm->therm_backup, catchup_temp);
		return catchup_temp;
	}
	/* Out of target time, doesn't scaling */
		...
		pr_info("catchup_skip ts_diff:%d, catchup_time_s:%d, "
			"temp:%d, therm_backup:%d, result_temp:%d\n",
			ts_diff, catchup_time_s,
			temp, smooth_therm->therm_backup, temp);
		return temp;
	}
}
...
skip_time:
	...
	if (smooth_therm.smooth_filter_enable) {
		if (smooth_therm.enable_flag) {
			smooth_therm.therm_backup = catchup_batt_therm(&smooth_therm, temp, ts_diff);
			smooth_therm.last_time = now;
		}
...
int override_fg_parse_ki_coefficient(struct fg_chip *chip, struct device_node *node)
{
	...
	if (chip->dt.dynamic_ki_en) {
		...
		pr_info("dynamic:enabled chg:%d dischg:%d soc_alert:%d "
				...
				ki_coeff.chg_enable, ki_coeff.dischg_enable,
				ki_coeff.soc_alert, ki_coeff.ki_comp,
				ki_coeff.margin_vfloat_mv,
				ki_coeff.ki_limit[MIN_KI],
				ki_coeff.ki_limit[MAX_KI]);
	}
...
static void polling_voltage_gap(struct work_struct *work)
{
	...
	slope_volt_gap = (total_gap*CHANGE_POINT) / (BACKUP_MAX_CNT*LOOP_TIME_S);
	real_slope = (total_gap*CHANGE_POINT) / (ki_coeff.loop_cnt*LOOP_TIME_S);
	...
	case POWER_SUPPLY_STATUS_CHARGING:
		...
		if (slope_volt_gap >= (CHANGE_POINT)/(quick_update ? SCALING_FACTOR : 1))
	...
	pr_info("direction=%d, backup=%d, tot_gap=%d, cnt=%d, slope=%d, real_slope=%d, vfloat=%d, stay=%d, alert=%d, charge=%s\n",
...
static int extension_fg_gen3_set_ki_coeff(struct fg_chip *chip)
{
	...
	if ((ki_coeff.backup_vote == VOTE_INIT) || (rescale.result < 0))
		return 0;
